...
use crate::{Integer, OptionRange, RangeSetInt, SafeSubtract};
...
pub struct SortedDisjointFromUnsortedDisjoint<T>
where
    T: Integer,
{
...
impl<T: Integer> SortedDisjointFromUnsortedDisjoint<T> {
    pub fn new<I>(unsorted_disjoint: UnsortedDisjoint<T, I>) -> Self
    where
        I: Iterator<Item = (T, T)>,
    {
        SortedDisjointFromUnsortedDisjoint {
...
impl<T: Integer> FromIterator<(T, T)> for SortedDisjointFromUnsortedDisjoint<T> {
    fn from_iter<I: IntoIterator<Item = (T, T)>>(iter: I) -> Self {
        Self::new(UnsortedDisjoint::new(iter.into_iter()))
    }
}

// Iterator for SortedDisjointFromUnsortedDisjoint
impl<T: Integer> Iterator for SortedDisjointFromUnsortedDisjoint<T> {
    type Item = (T, T);
