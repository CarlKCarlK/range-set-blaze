...
A crate for efficiently manipulating sets of integers using set operations such as `union()`, `intersection()`, and `difference()`. The integers can be any size (u8 to u128) and may be signed (i8 to i128). Internally, the integers are stored as ranges in the cache-efficient `BTreeMap`.

The crate differs from the standard `BTreeSet` and `HashSet` because it does not need to store every integer in the set, only for every range of integers. It differs from other interval libraries (that we know of) by being specialized and optimized for integer elements.
