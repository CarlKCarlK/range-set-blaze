...
The [`RangeSetBlaze::from_slice`][from-slice] constructor is always available (no feature flag
needed) and can speed up construction from array-like collections using SIMD where available, on
stable Rust, via `fearless_simd`. With `std` (the default), the best SIMD instructions for the
running CPU are detected at runtime; in `no_std`, the SIMD features selected at compile time for
the target are used.
