...
# Nightly is required by the optional float, cursor, and `doc_cfg` features.
show-docs:
...
# ============================================================================
# MSRV (matches `rust-version` in Cargo.toml — run `rustup toolchain install 1.89` once)
# ============================================================================

# Check the crate still compiles on the declared MSRV (1.89)
msrv-check:
    cargo +1.89 check --verbose
    cargo +1.89 check --verbose --no-default-features
    cargo +1.89 check --verbose --features std
