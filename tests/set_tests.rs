...
test_normal_and_wasm!(
    fn parity() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _parity: RangeSetBlaze<u8> =
            [[a.ranges()].intersection()].union().into_range_set_blaze();
...
test_normal_and_wasm!(
    fn range_set_int_constructors() {
        ...
        let a1: RangeSetBlaze<i32> =
            CheckSortedDisjoint::new([-10..=-5, 1..=2]).into_range_set_blaze();
        assert!(a0 == a1 && a0.to_string() == "-10..=-5, 1..=2");
        ...
        for a in [
            ...
            for start in [i128::MIN, i128::MIN + 1, 0, i128::MAX - 1, i128::MAX] {
                for end in [i128::MIN, i128::MIN + 1, 0, i128::MAX - 1, i128::MAX] {
                    ...
                    println!("{start}..={end}");
                    let slow =
                        panic::catch_unwind(AssertUnwindSafe(|| a._rogs_range_slow(start..=end)))
                            .ok();
...
test_normal_and_wasm!(
    fn test_every_sorted_disjoint_method() {
        ...
        let c1: RangeMapBlaze<i32, &str> =
            RangeMapBlaze::from_iter([(1..=2, &"a"), (5..=100, &"b")]);
        ...
        macro_rules! fresh_instances {
            () => {{
                ...
                let e: DynSortedDisjoint<_> = DynSortedDisjoint::new(
